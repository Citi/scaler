...
class SymphonyTaskManager(Looper, TaskManager):
    ...
    async def __execute_task(self, task: Task) -> asyncio.Future:
        ...
        function_bytes = await self._connector_storage.get_object(task.func_object_id)
        function = serializer.deserialize(function_bytes)

        arg_objects = []
        for arg in task.function_args:
            arg_bytes = await self._connector_storage.get_object(arg.data)
            arg_objects.append(serializer.deserialize(arg_bytes))
