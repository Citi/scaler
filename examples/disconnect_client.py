...
def main():
    ...
    # and all object reference will be invalidated. The client can reinstall tasks as it wishes.
    client.clear()
    ...
    # state will not be affected by the method.
    client.disconnect()

    # The user may also choose to shutdown the scheduler while disconnecting client from the scheduler. Such a request
    # is not guaranteed to succeed, scheduler can only be closed when it is not running under "protected" mode.
    # client.shutdown()
